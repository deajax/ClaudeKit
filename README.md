...
Claude Code 跨平台桌面管理工具，帮助开发者在不同模型商和 API 配置之间快速切换，内置终端集成与环境变量管理。

## 这是什么

ClaudeKit 是一个基于 Electron 的桌面应用，围绕 [Claude Code CLI](https://github.com/anthropics/claude-code) 提供图形化管理能力。允许你多个模型商之间切换，或者需要为不同项目配置不同的 Claude Code 环境时，ClaudeKit 将这些操作从命令行搬到可视化的工具栏中，一键完成切换和注入。
...
不使用传统数据库，所有数据以 JSON 文件形式存储在 `~/.ClaudeCLI/` 目录下：
...
用户数据存储在 `~/.ClaudeCLI/`，卸载应用不会自动删除此目录，如需清理请手动删除。
